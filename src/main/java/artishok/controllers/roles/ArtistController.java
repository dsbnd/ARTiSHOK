...
public class ArtistController {
    ...
    public ResponseEntity<?> getMyArtworks(
        ...
        try {
            ...
            if (status != null && !status.isEmpty()) {
                try {
                    ...
                    allArtworks = allArtworks.stream()
                            .filter(artwork -> artwork.getStatus() == artworkStatus)
                            .collect(Collectors.toList());
                } catch (IllegalArgumentException e) {
                    return ResponseEntity.badRequest()
                            .body(Map.of("error", "Некорректный статус произведения"));
                }
            }
            ...
            List<Map<String, Object>> artworkDTOs = paginatedArtworks.stream()
                    .map(this::convertArtworkToDTO)
                    .collect(Collectors.toList());

            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "artworks", artworkDTOs,
                    "total", allArtworks.size(),
    ...
    public ResponseEntity<?> createArtwork(@RequestBody Map<String, Object> artworkData) {
        try {
            ...
            if (!artworkData.containsKey("title") || !artworkData.containsKey("bookingId")) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Поля title и bookingId обязательны"));
            }
            ...
            try {
                ...
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Некорректный формат bookingId"));
            }
            ...
            if (!bookingOpt.isPresent()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(Map.of("error", "Бронирование не найдено"));
            }
            ...
            artwork.setDescription(artworkData.containsKey("description") ?
                    artworkData.get("description").toString() : "");
            ...
            if (artworkData.containsKey("creationYear")) {
                try {
                    ...
                    return ResponseEntity.badRequest()
                            .body(Map.of("error", "Некорректный год создания"));
                }
            }
            ...
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "message", "Произведение добавлено",
                    "artwork", convertArtworkToDTO(savedArtwork)
            ));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> updateArtwork(
        ...
        try {
            ...
            if (!artworkOpt.isPresent()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(Map.of("error", "Произведение не найдено"));
            }
            ...
            if (updates.containsKey("status")) {
                try {
                    ...
                    return ResponseEntity.badRequest()
                            .body(Map.of("error", "Некорректный статус"));
                }
            }
            ...
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "message", "Произведение обновлено",
                    "artwork", convertArtworkToDTO(updatedArtwork)
            ));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
    ...
    public ResponseEntity<?> deleteArtwork(@PathVariable Long id) {
        try {
            ...
            if (artwork.getStatus() == ArtworkStatus.PUBLISHED) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Нельзя удалить опубликованное произведение"));
            }
        ...
        try {
            ...
            if (status != null && !status.isEmpty()) {
                try {
                    ...
                    artistBookings = artistBookings.stream()
                            .filter(booking -> booking.getStatus() == bookingStatus)
                            .collect(Collectors.toList());
                } catch (IllegalArgumentException e) {
                    return ResponseEntity.badRequest()
                            .body(Map.of("error", "Некорректный статус бронирования"));
                }
            }
    ...
    public ResponseEntity<?> getActiveBookings() {
        try {
            ...
            // Преобразование в DTO
            List<Map<String, Object>> bookingDTOs = activeBookings.stream()
                    .map(this::convertBookingToDTO)
                    .collect(Collectors.toList());
    ...
    public ResponseEntity<?> createBooking(@RequestBody Map<String, Object> bookingData) {
        try {
            ...
            if (!bookingData.containsKey("exhibitionStandId")) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Поле exhibitionStandId обязательно"));
            }
            ...
            try {
                ...
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Некорректный формат exhibitionStandId"));
            }
            ...
            if (exhibition.getStatus() != ExhibitionStatus.ACTIVE) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Выставка не активна для бронирования"));
            }

            // Проверяем даты выставки
            if (exhibition.getEndDate().isBefore(LocalDateTime.now())) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Выставка уже завершена"));
            }

            if (exhibition.getStartDate().isAfter(LocalDateTime.now())) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Выставка еще не началась"));
            }
            ...
            if (!standBookings.isEmpty()) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Стенд уже забронирован"));
            }
            ...
            return ResponseEntity.ok(Map.of(
                    ...
                    "booking", convertBookingToDTO(savedBooking)
            ));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
    ...
    public ResponseEntity<?> getAvailableExhibitions() {
        try {
            ...
            // Преобразование в DTO
            List<Map<String, Object>> exhibitionDTOs = activeExhibitions.stream()
    ...
    public ResponseEntity<?> getAvailableStands(@PathVariable Long id) {
        try {
            ...
            if (exhibition.getStatus() != ExhibitionStatus.ACTIVE) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Выставка не активна"));
            }
            ...
            List<ExhibitionStand> availableStands = allStands.stream()
                    .filter(stand -> {
                        boolean isBooked = allBookings.stream()
                                .anyMatch(booking ->
                                        booking.getExhibitionStand().getId().equals(stand.getId()) &&
                                                (booking.getStatus() == BookingStatus.CONFIRMED ||
            ...
            // Преобразование в DTO
            List<Map<String, Object>> standDTOs = availableStands.stream()
